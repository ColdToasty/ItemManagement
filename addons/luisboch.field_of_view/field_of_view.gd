...
export var target_groups = ["Player"]
...
export var frequency = 0.01 setget _set_frequency
var timer
...
func check_view():
	...
	for tg_point in points_arc:
		...
		var result = space_state.intersect_ray(get_global_transform().origin, to_global(tg_point.pos), [get_parent()], collision_mask, true, true)
		if not result: 
			continue
		if result:
